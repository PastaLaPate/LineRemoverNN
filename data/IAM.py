...
def split_into_blocks(image, block_size=512, transform=None):
...
def reconstruct_image(blocks, original_height, original_width, block_size=512):
    ...
    reconstructed_padded_image = torch.zeros((channels, padded_height, padded_width), dtype=blocks[0].dtype)
    
    # Iterate through blocks and reconstruct
    block_index = 0
    for i in range(0, padded_height, block_size):
        for j in range(0, padded_width, block_size):
            reconstructed_padded_image[:, i:i + block_size, j:j + block_size] = blocks[block_index]
            block_index += 1
    
    # Crop to the original dimensions
    reconstructed_image = reconstructed_padded_image[:, :original_height, :original_width]
    return reconstructed_image

class IAMPages(Dataset):
    def __init__(self, lines_pages_dir, nolines_pages_dir, json_dir, random=False, splitSquare=False, transform=None, target_transform=None):
        ...
        if not (len(os.listdir(self.lines_pages_dir)) == len(os.listdir(self.nolines_pages_dir)) == len(os.listdir(self.json_dir))):
        ...
        self.blocksPerImage = math.ceil(self.IMG_Height / 512) * math.ceil(self.IMG_WIDTH / 512)
    ...
    def __getitem__(self, index):
        ...
        if self.splitSquare:
            linesImages = split_into_blocks(linesImage, transform=self.transform)[origIndex%self.blocksPerImage]
            noLinesImages = split_into_blocks(noLinesImage, transform=self.transform)[origIndex%self.blocksPerImage]
            sample = {'lines': linesImages, 'noLines': noLinesImages, 'jsonData': jsonRead, 'shape': linesImage.shape}
            return sample
        if self.transform:
...
if __name__=='__main__':
    ...
    dataset = IAMPages(pages_dir, nolines_dir, json_dir, random=False, splitSquare=False)
    cerLines = 0
    cerNoLines = 0
    wordsN = 0
    for i in tqdm.tqdm(range(50)):
        ...
        #plt.imshow(img)
        #plt.show()
    print(f"Lines image Avg CER : {(cerLines/wordsN)*100}% No lines image Avg CER : {(cerNoLines/wordsN)*100}%")
