...
public:
  AsyncDisplayer(BaseDisplay *parent, std::ostream *out, Duration interval,
                 bool no_tty)
...
template <typename T> struct Provider<std::atomic<T>> {
  using value_type = T;
...
template <typename T> struct Provider<std::function<T()>> {
  using value_type = T;
...
template <typename T, typename = void> struct ReasonablyInvocable {
...
template <typename T, typename = void> struct ProviderSelector {
  using type_t = Provider<T>;
};

template <typename T> struct ProviderSelector<std::atomic<T>> {
...
template <typename T> using provider_t = typename ProviderSelector<T>::type_t;

/// Trait class to extract underlying value type from numerics and
/// std::atomics of numerics.
template <typename T> struct AtomicTraits {
  using value_type = T;
};

template <typename T> struct AtomicTraits<std::atomic<T>> {
  using value_type = T;
};

template <typename T> struct AtomicTraits<Provider<T>> {
  using value_type = typename Provider<T>::value_type;
};

template <typename T> struct AtomicTraits<Provider<std::atomic<T>>> {
  using value_type = typename Provider<std::atomic<T>>::value_type;
};

template <typename T> struct AtomicTraits<Provider<std::function<T()>>> {
  using value_type = typename Provider<std::function<T()>>::value_type;
};

template <typename T> using value_t = typename AtomicTraits<T>::value_type;

template <typename T>
using signed_t =
    typename std::conditional_t<std::is_integral_v<T>, std::make_signed<T>,
                                std::common_type<T>>::type;

/// Helper class to measure and display speed of progress.
template <typename ProgressProvider> class Speedometer {
private:
...
public:
  double speed() {
    ...
    if (duration_increment_sum_.count() == 0) {
      return 0;
    }
    double base_speed =
...
protected:
  ...
  long render_(bool /*redraw*/ = false, const std::string &end = " ") override {
    ...
    if (not format_.empty()) {
      ...
      if (speedom_) {
        fmt::print(out(), fmt::runtime(format_), "value"_a = progress,
                   "speed"_a = speedom_->speed(), "red"_a = red,
                   "green"_a = green, "yellow"_a = yellow, "blue"_a = blue,
                   "magenta"_a = magenta, "cyan"_a = cyan, "reset"_a = reset);
      } else {
        fmt::print(out(), fmt::runtime(format_), "value"_a = progress,
                   "red"_a = red, "green"_a = green, "yellow"_a = yellow,
                   "blue"_a = blue, "magenta"_a = magenta, "cyan"_a = cyan,
                   "reset"_a = reset);
      }
    ...
    if (not format_.empty()) {
      ...
      out() << std::vformat(format_, std::make_format_args(progress,
                                                           speed,   // 1
                                                           red,     // 2
                                                           green,   // 3
                                                           yellow,  // 4
...
public:
  ...
  CounterDisplay(provider_t<Progress> progress_provider,
      ...
      : BaseDisplay(cfg.out, as_duration(cfg.interval), cfg.message,
                    cfg.format.empty() ? "" : cfg.format + " ", cfg.no_tty),
...
template <
    typename Progress, typename ProgressProvider = provider_t<Progress>,
...
template <typename ValueType> struct ProgressBarConfig {
  std::ostream *out = &std::cout; ///< output stream
...
template <typename Progress> class ProgressBarDisplay : public BaseDisplay {
protected:
  using ProgressProvider = provider_t<Progress>;
  using ValueType = value_t<ProgressProvider>;
...
protected:
  ...
  long render_(bool /*redraw*/ = false, const std::string &end = " ") override {
    ...
    if (not format_.empty()) {
      ...
      if (speedom_) {
        fmt::print(out(), fmt::runtime(format_), "value"_a = progress,
                   "bar"_a = bar_ss.str(), "percent"_a = percent,
                   "total"_a = total_, "speed"_a = speedom_->speed(),
                   "red"_a = red, "green"_a = green, "yellow"_a = yellow,
                   "blue"_a = blue, "magenta"_a = magenta, "cyan"_a = cyan,
                   "reset"_a = reset);
      } else {
        fmt::print(out(), fmt::runtime(format_), "value"_a = progress,
                   "bar"_a = bar_ss.str(), "percent"_a = percent,
                   "total"_a = total_, "red"_a = red, "green"_a = green,
                   "yellow"_a = yellow, "blue"_a = blue, "magenta"_a = magenta,
                   "cyan"_a = cyan, "reset"_a = reset);
      }
      return std::count(format_.begin(), format_.end(), '\n');
    }
#elif defined(BARKEEP_ENABLE_STD_FORMAT)
...
public:
  ...
  ProgressBarDisplay(provider_t<Progress> progress_provider,
      ...
      : BaseDisplay(cfg.out, as_duration(cfg.interval), cfg.message,
                    cfg.format.empty() ? "" : cfg.format + " ", cfg.no_tty),
        progress_provider_(std::move(progress_provider)),
        speed_unit_(cfg.speed_unit), total_(cfg.total) {
...
template <typename ValueType> struct IterableBarConfig {
  std::ostream *out = &std::cout; ///< output stream
  std::string format = "";        ///< format string for the entire progress bar
  std::string message = "";       ///< message to display with the bar
...
template <typename Container> class IterableBar {
public:
...
public:
  ...
  public:
    Iterator(typename Container::iterator it, typename Container::iterator end,
             ProgressType &idx, std::shared_ptr<Bar> bar)
  ...
  IterableBar(Container &container,
      ...
      : container_(container), idx_(std::make_shared<ProgressType>(0)),
        bar_(std::make_shared<Bar>(
            &*idx_,
            ProgressBarConfig<ValueType>{cfg.out, container.size(), cfg.format,
                                         cfg.message, cfg.speed, cfg.speed_unit,
                                         cfg.style, cfg.interval, cfg.no_tty,
                                         /*show=*/false})) {}
